...
interface AreaRow { area: string; demand: number; supply: number; uncovered: boolean; lat?: number; lng?: number }
...
/**
 * Leaflet map. The primary layer is the actual postcode AREAS — one dot per area
 * placed at its real centroid, green where a repairer covers it and red where
 * there's demand but nobody covering. Job locations underlay as a heat cloud, and
 * repairer bases show with a faint travel-radius circle (the radius is only a
 * fallback matching signal, not the covered geography).
 */
function CoverageMap({ areas, jobs, repairers }: { areas: AreaRow[]; jobs: [number, number][]; repairers: RepairerPoint[] }) {
  ...
  useEffect(() => {
    ...
    // Job demand → heat layer (underlay).
    if (jobs.length) {
      const heat = (L as unknown as { heatLayer: (pts: [number, number][], opts: object) => L.Layer })
        .heatLayer(jobs, { radius: 28, blur: 20, minOpacity: 0.3, max: 3 });
      layers.addLayer(heat);
    }

    // Repairer bases → small markers + faint travel-radius circle (fallback signal).
    for (const r of repairers) {
      if (r.radiusKm) {
        layers.addLayer(L.circle([r.lat, r.lng], {
          radius: r.radiusKm * 1000, color: '#94a3b8', weight: 1, dashArray: '4', fillColor: '#94a3b8', fillOpacity: 0.04,
        }));
      }
      layers.addLayer(
        L.circleMarker([r.lat, r.lng], { radius: 4, color: '#334155', weight: 1, fillColor: '#64748b', fillOpacity: 0.9 })
          .bindPopup(`<strong>${r.name}</strong>${r.radiusKm ? `<br/>travel radius ~${r.radiusKm} km (fallback)` : ''}`),
      );
    }

    // Covered postcode areas → the primary layer. One dot per area at its centroid,
    // sized by demand, green = covered, red = demand with no coverage.
    const maxDemand = Math.max(1, ...areas.map((a) => a.demand));
    const placed = areas.filter((a) => a.lat != null && a.lng != null);
    for (const a of placed) {
      const covered = a.supply > 0;
      const size = 8 + 14 * Math.sqrt(a.demand / maxDemand); // area-proportional-ish
      layers.addLayer(
        L.circleMarker([a.lat!, a.lng!], {
          radius: size,
          color: covered ? '#065f46' : '#991b1b',
          weight: 2,
          fillColor: covered ? '#10b981' : '#ef4444',
          fillOpacity: 0.55,
        }).bindPopup(
          `<strong>${a.area}</strong><br/>${a.demand} job${a.demand === 1 ? '' : 's'} · ${a.supply} repairer${a.supply === 1 ? '' : 's'} covering${covered ? '' : '<br/><em>uncovered</em>'}`,
        ).bindTooltip(a.area, { permanent: true, direction: 'center', className: 'area-label' }),
      );
    }

    // Fit to whatever we've got.
    const all: L.LatLngExpression[] = [
      ...placed.map((a) => [a.lat!, a.lng!] as [number, number]),
      ...jobs,
      ...repairers.map((r) => [r.lat, r.lng] as [number, number]),
    ];
    if (all.length) map.fitBounds(L.latLngBounds(all).pad(0.2), { maxZoom: 9 });
  }, [areas, jobs, repairers]);
...
export function CoveragePage() {
  ...
  return (
    <Layout>
      ...
      <p className="text-sm text-gray-500 mb-6">Covered postcode areas (green) vs demand with no coverage (red), over a heat cloud of job locations.</p>
      ...
      {isLoading || !data ? (
        ...
        <div className="space-y-6">
          ...
          <Card>
            <CardBody>
              <CoverageMap areas={data.areas} jobs={data.points.jobs} repairers={data.points.repairers} />
              <div className="flex flex-wrap items-center gap-4 mt-3 text-xs text-gray-500">
                <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-full bg-emerald-500 border-2 border-emerald-800" /> covered area</span>
                <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-full bg-red-500 border-2 border-red-800" /> uncovered demand</span>
                ...
                <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-full bg-slate-500 border border-slate-700" /> repairer base + travel radius (fallback)</span>
